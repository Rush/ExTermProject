...
void VT100Parser::decodeOSC(uchar character)
{
        if (!parameters.size() &&
            ...
            {
                if (character == 0x07 || character == 0x5c) {
                    if (decodeOscState == ChangeWindowAndIconName ||
                        decodeOscState == ChangeWindowTitle) {
                        QString title = QString(currentData.mid(currentTokenStart+4,
                                                                             currentPosition - currentTokenStart -1));
